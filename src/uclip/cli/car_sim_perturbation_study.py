"""car_sim perturbation study combining noise and resolution degradations."""
...
from .car_sim_driver import CarSimRoutineConfig, run as run_car_sim
...
def make_downsample_transform(target_short_side: int) -> Callable[[Image.Image], Image.Image]:
    def transform(image: Image.Image) -> Image.Image:
        width, height = image.size
        short_side = min(width, height)
        if target_short_side <= 0 or target_short_side >= short_side:
            return image
        scale = target_short_side / float(short_side)
        new_width = max(1, int(round(width * scale)))
        new_height = max(1, int(round(height * scale)))
        downsampled = image.resize((new_width, new_height), Image.BILINEAR)
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run noise and resolution perturbation study on car_sim")
    ...
    parser.add_argument("--out-root", type=str, default="runs/car_sim_perturbations_noise_res_v1")
    parser.add_argument("--noise-stds", type=str, default="0.0,0.05,0.1,0.2,0.35")
    parser.add_argument("--downsample-sizes", type=str, default="144,104,72")
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    for scenario in scenarios:
        ...
        save_json(
            scenario_out / "perturbation_summary.json",
            {
                "scenario": scenario.metadata,
                "car_sim_summary": summary,
